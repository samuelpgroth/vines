...
def penetrable_circle(k0, k1, rad, plot_grid):
    ...
    n_terms = np.max([200, np.int(55 + (k0 * a)**1.01)])
...
# FIXME: attempt at numba implementation. Not working currently.
def penetrable_circle_numba(k0, k1, rad, plot_grid):
    # from pylab import find
    from scipy.special import jv, hankel1
    import numpy as np
    x = np.vstack((plot_grid[0].ravel(), plot_grid[1].ravel()))
    ...
    @njit(parallel=True)
    def summation(k0):
        u_int_arr = np.zeros((npts, 2*n_terms+1), dtype=np.complex128)
        u_ext_arr = np.zeros((npts, 2*n_terms+1), dtype=np.complex128)
        for n in prange(-n_terms, n_terms):
            bessel_k0 = jv(n, k0 * rad)
            bessel_k1 = jv(n, k1 * rad)
            ...
            u_ext_arr[:, n] = a_n * hankel1(n, k0 * r) * np.exp(1j * n * theta)
            u_int_arr[:, n] = b_n * jv(n, k1 * r) * np.exp(1j * n * theta)

            # u_ext += a_n * hankel1(n, k0 * r) * np.exp(1j * n * theta)
            # u_int += b_n * jv(n, k1 * r) * np.exp(1j * n * theta)

        u_ext = np.sum(u_ext_arr, axis=1)
        u_int = np.sum(u_int_arr, axis=1)
        return u_ext, u_int

    u_ext, u_int = summation(k0)
